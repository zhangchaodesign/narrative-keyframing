...
const CATEGORY_LABEL = "Physiology";
const CATEGORY_GUIDANCE =
  "Physical appearance, clothing, body language, visible characteristics.";
...
export async function POST(request: Request) {
  try {
    ...
    const outlineText = storyOutline
      .map(
        (event, index) => `${index + 1}. ${event.label}: ${event.description}`,
      )
      .join("\n");
    ...
    const resolvedCharacterName = characterName?.trim().length
      ? characterName.trim()
      : "the character";
...
Return JSON that matches the provided schema.`;
    ...
    console.log("Character brainstorming prompt:", prompt);
    console.log("Brainstormed physiology traits:", object.traits);
