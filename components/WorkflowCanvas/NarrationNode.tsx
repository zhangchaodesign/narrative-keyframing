...
export function NarrationNode({ data }: NodeProps<NarrationNodeType>) {
  return (
    <div className="relative flex h-44 w-56 flex-col rounded-md border border-indigo-400 bg-indigo-50 p-3 text-xs shadow-sm">
      <div className="text-[10px] font-semibold uppercase tracking-wide text-indigo-500">
        {data?.narrator}
      </div>
      <div className="mt-2 flex-1 overflow-y-auto rounded border border-indigo-100 bg-white/70 p-2 text-[11px] leading-snug text-indigo-900">
        {data?.reflection}
      </div>
      <CustomHandle
        type="target"
